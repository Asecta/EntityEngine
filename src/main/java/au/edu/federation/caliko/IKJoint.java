...
public class IKJoint implements Serializable {
     ...
     * <p>
     * Internally, this uses the copy constructor {@link #IKJoint(IKJoint)} to
     * perform the clone.
     *
     * @param source The FabrikJoint3D to clone.
     * @return The cloned FabrikJoint3D.
     */
    public IKJoint clone(IKJoint source) {
        return new IKJoint(source);
    }

    /**
     * Set the constraint angles of this joint to match those of a source joint, essentially making a copy of the source joint.
     *
     * @param    source    The source joint from which to copy values.
     */

    public void set(IKJoint source) {
        // Copy by value
    ...
    public void setAsBallJoint(float constraintAngleDegs) {
        ...
        IKJoint.validateConstraintAngleDegs(constraintAngleDegs);
    ...
    public void setHinge(JointType jointType, Vector3f rotationAxis, float clockwiseConstraintDegs, float anticlockwiseConstraintDegs, Vector3f referenceAxis) {
        ...
        if (!Utils.approximatelyEquals(rotationAxis.dot(referenceAxis), 0.0f, 0.01f)) {
            float angleDegs = VectorUtils.getAngleBetweenDegs(rotationAxis, referenceAxis);
        ...
        IKJoint.validateConstraintAngleDegs(clockwiseConstraintDegs);
        IKJoint.validateConstraintAngleDegs(anticlockwiseConstraintDegs);
        IKJoint.validateAxis(rotationAxis);
        IKJoint.validateAxis(referenceAxis);
        ...
        mRotationAxisUV.set(rotationAxis.normalize(new Vector3f()));
        mReferenceAxisUV.set(referenceAxis.normalize(new Vector3f()));
    }
     ...
     * <p>
     * Providing a zero Vector3f for any axis, or constraint angles outside the range 0..180 degrees results in an IllegalArgumentException.
     *
    ...
    public void setBallJointConstraintDegs(float angleDegs) {
        IKJoint.validateConstraintAngleDegs(angleDegs);
    ...
    public void setHingeRotationAxis(Vector3f axis) {
        IKJoint.validateAxis(axis);

        if (mJointType != JointType.BALL) {
            mRotationAxisUV.set(axis.normalize(new Vector3f()));
        } else {
    ...
    public boolean equals(Object obj) {
        ...
        IKJoint other = (IKJoint) obj;
        if (Float.floatToIntBits(mHingeAnticlockwiseConstraintDegs) != Float.floatToIntBits(other.mHingeAnticlockwiseConstraintDegs)) {
            return false;
        }
        if (Float.floatToIntBits(mHingeClockwiseConstraintDegs) != Float.floatToIntBits(other.mHingeClockwiseConstraintDegs)) {
            return false;
        }
