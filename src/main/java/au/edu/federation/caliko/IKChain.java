...
import au.edu.federation.caliko.IKJoint.JointType;
 ...
 * <p>
 * A FabrikChain3D consists primarily of a list of connected {@link IKBone} objects, and a number of parameters which
...
public class IKChain implements Serializable {
     ...
     */
    private List<IKBone> mChain = new ArrayList<>();
     ...
     * <p>
     * See {@link #addBone(IKBone)}
     * See {@link #addConsecutiveBone(IKBone)}
     * See {@link #removeBone(int)}
     */
    private float mChainLength;

    /**
     * mBaseLocation (Vector3f)	The location of the start joint of the first bone in the IK chain.
     * <p>
     ...
     * <p>
     * The default is Vector3f(0.f, 0.0f).
     * <p>
    ...
    /**
     * Each chain has a BaseboneConstraintType - this may be either:
     ...
     */
    private BaseboneConstraintType mBaseboneConstraintType = BaseboneConstraintType.NONE;

    /**
     * mBaseboneConstraintUV	The direction around which we should constrain the basebone.
     * <p>
     * To ensure correct operation, the provided Vector3f is normalised inside the {@link #setBaseboneConstraintUV(Vector3f)} method. Passing a Vector3f
     ...
     * <p>
     * The target location can be updated via the {@link #solveForTarget(Vector3f)} or {@link #solveForTarget(float, float, float)} methods, which in turn
     * will call the solveIK(Vector3f) method to attempt to solve the IK chain, resulting in an updated chain configuration.
     * <p>
     * The default is Vector3f(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE)
     */
     ...
     * {@link #solveForTarget(Vector3f)} or (@link #solveForTarget(float, float, float) methods.
     */
    ...
    public IKChain(IKChain source) {
        ...
        if (source.mBaseboneConstraintType != BaseboneConstraintType.NONE) {
    ...
    public void addConsecutiveBone(Vector3f directionUV, float length) {
        ...
        if (!mChain.isEmpty()) {
            ...
            addBone(new IKBone(prevBoneEnd, directionUV.normalize(new Vector3f()), length));
    ...
    public void addConsecutiveBone(IKBone bone) {
        // Validate the direction unit vector - throws an IllegalArgumentException if it has a magnitude of zero
        ...
        if (!this.mChain.isEmpty()) {
            // Get the end location of the last bone, which will be used as the start location of the new bone
            Vector3f prevBoneEnd = new Vector3f(mChain.get(this.mChain.size() - 1).getEndLocation());

            bone.setStartLocation(prevBoneEnd);

            Vector3f dest = new Vector3f();
            prevBoneEnd.add(dir.mul(len, dest), dest);

            bone.setEndLocation(dest);
    ...
    public void addConsecutiveFreelyRotatingHingedBone(Vector3f directionUV, float length, JointType jointType, Vector3f hingeRotationAxis) {
        ...
        addConsecutiveHingedBone(directionUV, length, jointType, hingeRotationAxis, 180.0f, 180.0f, VectorUtils.genPerpendicularVectorQuick(hingeRotationAxis));
    }
     ...
     */
    public void addConsecutiveHingedBone(Vector3f directionUV, float length, JointType jointType, Vector3f hingeRotationAxis, float clockwiseDegs, float anticlockwiseDegs, Vector3f hingeReferenceAxis) {
        ...
        IKBone bone = new IKBone(prevBoneEnd, directionUV, length);
    ...
    public void addConsecutiveRotorConstrainedBone(Vector3f boneDirectionUV, float boneLength, float constraintAngleDegs) {
        ...
        IKBone bone = new IKBone(mChain.get(mChain.size() - 1).getEndLocation(), boneDirectionUV.normalize(new Vector3f()), boneLength);
    ...
    public Vector3f getBaseboneRelativeConstraintUV() {
        return mBaseboneRelativeConstraintUV;
    }
     ...
     * @return The basebone constraint type of this chain.
     */
    public BaseboneConstraintType getBaseboneConstraintType() {
        return mBaseboneConstraintType;
    }
    ...
    public Vector3f getBaseboneConstraintUV() {
        if (mBaseboneConstraintType != BaseboneConstraintType.NONE) {
     ...
     */
    public Vector3f getBaseLocation() {
    ...
    public IKBone getBone(int boneNumber) {
    ...
    public Vector3f getEffectorLocation() {
    ...
    public Vector3f getEmbeddedTarget() {
        return mEmbeddedTarget;
    }
    ...
    public Vector3f getLastTargetLocation() {
        return mLastTargetLocation;
    }
     ...
     **/
    void setBaseboneRelativeConstraintUV(Vector3f constraintUV) {
        mBaseboneRelativeConstraintUV = constraintUV;
    }
     ...
     **/
    void setBaseboneRelativeReferenceConstraintUV(Vector3f constraintUV) {
        mBaseboneRelativeReferenceConstraintUV = constraintUV;
    }
     ...
     * @return The relative basebone reference constraint unit vector.
     */
    public Vector3f getBaseboneRelativeReferenceConstraintUV() {
        return mBaseboneRelativeReferenceConstraintUV;
    }
     ...
     */
    public void setRotorBaseboneConstraint(BaseboneConstraintType rotorType, Vector3f constraintAxis, float angleDegs) {
        // Sanity checking
        if (mChain.isEmpty()) {
        ...
        if (!(rotorType == BaseboneConstraintType.GLOBAL_ROTOR || rotorType == BaseboneConstraintType.LOCAL_ROTOR)) {
        ...
        mBaseboneConstraintUV = constraintAxis.normalize(new Vector3f());
     ...
     */
    public void setHingeBaseboneConstraint(BaseboneConstraintType hingeType, Vector3f hingeRotationAxis, float cwConstraintDegs, float acwConstraintDegs, Vector3f hingeReferenceAxis) {
        // Sanity checking
        if (mChain.isEmpty()) {
            throw new RuntimeException("Chain must contain a basebone before we can specify the basebone constraint type.");
        }
        ...
        if (!(VectorUtils.perpendicular(hingeRotationAxis, hingeReferenceAxis))) {
        ...
        mBaseboneConstraintUV.set(hingeRotationAxis.normalize(new Vector3f()));

        IKJoint hinge = new IKJoint();
    ...
    public void setFreelyRotatingGlobalHingedBasebone(Vector3f hingeRotationAxis) {
        setHingeBaseboneConstraint(BaseboneConstraintType.GLOBAL_HINGE, hingeRotationAxis, 180.0f, 180.0f, VectorUtils.genPerpendicularVectorQuick(hingeRotationAxis));
    }
    ...
    public void setFreelyRotatingLocalHingedBasebone(Vector3f hingeRotationAxis) {
        setHingeBaseboneConstraint(BaseboneConstraintType.LOCAL_HINGE, hingeRotationAxis, 180.0f, 180.0f, VectorUtils.genPerpendicularVectorQuick(hingeRotationAxis));
    }
    ...
    public void setLocalHingedBasebone(Vector3f hingeRotationAxis, float cwDegs, float acwDegs, Vector3f hingeReferenceAxis) {
        setHingeBaseboneConstraint(BaseboneConstraintType.LOCAL_HINGE, hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis);
    }
    ...
    public void setGlobalHingedBasebone(Vector3f hingeRotationAxis, float cwDegs, float acwDegs, Vector3f hingeReferenceAxis) {
        setHingeBaseboneConstraint(BaseboneConstraintType.GLOBAL_HINGE, hingeRotationAxis, cwDegs, acwDegs, hingeReferenceAxis);
    }
     ...
     * @see IKJoint#setBallJointConstraintDegs(float angleDegs)
     * @see IKJoint#setHingeJointClockwiseConstraintDegs(float)
     * @see IKJoint#setHingeJointAnticlockwiseConstraintDegs(float)
     */
     ...
     * we attach to the other chain are the same Vector3f object.
     * <p>
    ...
    public void setBaseLocation(Vector3f baseLocation) {
        mFixedBaseLocation = baseLocation;
    }
     ...
     */
    public void connectToStructure(IKStructure structure, int chainNumber, int boneNumber) {
     ...
     * BaseboneConstraintType.GLOBAL_ABSOLUTE will result in a RuntimeException being thrown.	 *
     *
    ...
    public void setFixedBaseMode(boolean value) {
        ...
        if (mBaseboneConstraintType == BaseboneConstraintType.GLOBAL_ROTOR && !value) {
            throw new RuntimeException("Cannot set a non-fixed base mode when the chain's constraint type is BaseboneConstraintType.GLOBAL_ABSOLUTE_ROTOR.");
        }
     ...
     * <p>
     * The embedded target location can be updated by calling updateEmbeddedTarget(Vector3f).
     *
    ...
    public float solveForTarget(Vector3f newTarget) {
        ...
        if (mLastTargetLocation.equals(newTarget, 0.001f) && mLastBaseLocation.equals(getBaseLocation(), 0.001f)) {
        ...
        List<IKBone> bestSolution = new ArrayList<>();
    ...
    private float solveIK(Vector3f target) {
        ...
        for (int loop = mChain.size() - 1; loop >= 0; --loop) {
            ...
            IKBone thisBone = mChain.get(loop);
            float thisBoneLength = thisBone.length();
            IKJoint thisBoneJoint = thisBone.getJoint();
            ...
            if (loop != mChain.size() - 1) {
                ...
                Vector3f outerBoneOuterToInnerUV = mChain.get(loop + 1).getDirectionUV().negate(new Vector3f());

                // Get the outer-to-inner unit vector of this bone
                Vector3f thisBoneOuterToInnerUV = thisBone.getDirectionUV().negate(new Vector3f());
                ...
                if (thisBoneJointType == JointType.BALL) {
                    ...
                    float angleBetweenDegs = VectorUtils.getAngleBetweenDegs(outerBoneOuterToInnerUV, thisBoneOuterToInnerUV);
                    float constraintAngleDegs = thisBoneJoint.getBallJointConstraintDegs();
                    if (angleBetweenDegs > constraintAngleDegs) {
                        thisBoneOuterToInnerUV = VectorUtils.getAngleLimitedUnitVectorDegs(thisBoneOuterToInnerUV, outerBoneOuterToInnerUV, constraintAngleDegs);
                    }
                    ...
                    thisBoneOuterToInnerUV = VectorUtils.projectOntoPlane(thisBoneOuterToInnerUV, thisBoneJoint.getHingeRotationAxis());
                    ...
                    if (loop > 0) {
                        m = VectorUtils.createRotationMatrix(mChain.get(loop - 1).getDirectionUV());
                        relativeHingeRotationAxis = VectorUtils.matMulVec(m, thisBoneJoint.getHingeRotationAxis()).normalize();
                    ...
                    // Note: The returned vector is normalised.
                    thisBoneOuterToInnerUV = VectorUtils.projectOntoPlane(thisBoneOuterToInnerUV, relativeHingeRotationAxis);
            ...
            } else // If we ARE working on the end effector bone...
            {
                ...
                switch (thisBoneJointType) {
                    ...
                    case LOCAL_HINGE:
                        ...
                        Matrix3f m = VectorUtils.createRotationMatrix(mChain.get(loop - 1).getDirectionUV());

                        // ...and transform the hinge rotation axis into the previous bones frame of reference.
                        Vector3f relativeHingeRotationAxis = VectorUtils.matMulVec(m, thisBoneJoint.getHingeRotationAxis()).normalize();
        ...
        for (int loop = 0; loop < mChain.size(); ++loop) {
            ...
            if (loop != 0) {
                ...
                if (jointType == JointType.BALL) {
                    float angleBetweenDegs = VectorUtils.getAngleBetweenDegs(prevBoneInnerToOuterUV, thisBoneInnerToOuterUV);
                    float constraintAngleDegs = thisBoneJoint.getBallJointConstraintDegs();

                    // Keep this bone direction constrained within the rotor about the previous bone direction
                    if (angleBetweenDegs > constraintAngleDegs) {
                        thisBoneInnerToOuterUV = VectorUtils.getAngleLimitedUnitVectorDegs(thisBoneInnerToOuterUV, prevBoneInnerToOuterUV, constraintAngleDegs);
                    }
                    ...
                    thisBoneInnerToOuterUV = VectorUtils.projectOntoPlane(thisBoneInnerToOuterUV, hingeRotationAxis);
                    ...
                    if (!(Utils.approximatelyEquals(cwConstraintDegs, -IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.001f)) && !(Utils.approximatelyEquals(acwConstraintDegs, IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.001f))) {
                        ...
                        float signedAngleDegs = VectorUtils.getSignedAngleBetweenDegs(hingeReferenceAxis, thisBoneInnerToOuterUV, hingeRotationAxis);
                        ...
                        if (signedAngleDegs > acwConstraintDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.rotateAboutAxisDegs(hingeReferenceAxis, acwConstraintDegs, hingeRotationAxis).normalize();
                        } else if (signedAngleDegs < cwConstraintDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.rotateAboutAxisDegs(hingeReferenceAxis, cwConstraintDegs, hingeRotationAxis).normalize();
                        }
                    }
                    ...
                    Matrix3f m = VectorUtils.createRotationMatrix(prevBoneInnerToOuterUV);

                    // Transform the hinge rotation axis into the previous bone's frame of reference
                    Vector3f relativeHingeRotationAxis = VectorUtils.matMulVec(m, hingeRotationAxis).normalize();

                    // Project this bone direction onto the plane described by the hinge rotation axis
                    // Note: The returned vector is normalised.
                    thisBoneInnerToOuterUV = VectorUtils.projectOntoPlane(thisBoneInnerToOuterUV, relativeHingeRotationAxis);
                    ...
                    if (!(Utils.approximatelyEquals(cwConstraintDegs, -IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.001f)) && !(Utils.approximatelyEquals(acwConstraintDegs, IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.001f))) {
                        ...
                        Vector3f relativeHingeReferenceAxis = VectorUtils.matMulVec(m, thisBoneJoint.getHingeReferenceAxis()).normalize();

                        // Get the signed angle (about the hinge rotation axis) between the hinge reference axis and the hinge-rotation aligned bone UV
                        // Note: ACW rotation is positive, CW rotation is negative.
                        float signedAngleDegs = VectorUtils.getSignedAngleBetweenDegs(relativeHingeReferenceAxis, thisBoneInnerToOuterUV, relativeHingeRotationAxis);

                        // Make our bone inner-to-outer UV the hinge reference axis rotated by its maximum clockwise or anticlockwise rotation as required
                        if (signedAngleDegs > acwConstraintDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.rotateAboutAxisDegs(relativeHingeReferenceAxis, acwConstraintDegs, relativeHingeRotationAxis).normalize();
                        } else if (signedAngleDegs < cwConstraintDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.rotateAboutAxisDegs(relativeHingeReferenceAxis, cwConstraintDegs, relativeHingeRotationAxis).normalize();
                        }
                    }
            ...
            } else // If we ARE working on the basebone...
            {
                ...
                {
                    if (mBaseboneConstraintType == BaseboneConstraintType.GLOBAL_ROTOR) {
                        // Get the inner-to-outer direction of this bone
                        Vector3f thisBoneInnerToOuterUV = thisBone.getDirectionUV();

                        float angleBetweenDegs = VectorUtils.getAngleBetweenDegs(mBaseboneConstraintUV, thisBoneInnerToOuterUV);
                        float constraintAngleDegs = thisBone.getBallJointConstraintDegs();

                        if (angleBetweenDegs > constraintAngleDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.getAngleLimitedUnitVectorDegs(thisBoneInnerToOuterUV, mBaseboneConstraintUV, constraintAngleDegs);
                        }
                    ...
                    } else if (mBaseboneConstraintType == BaseboneConstraintType.LOCAL_ROTOR) {
                        ...
                        float angleBetweenDegs = VectorUtils.getAngleBetweenDegs(mBaseboneRelativeConstraintUV, thisBoneInnerToOuterUV);
                        float constraintAngleDegs = thisBone.getBallJointConstraintDegs();
                        if (angleBetweenDegs > constraintAngleDegs) {
                            thisBoneInnerToOuterUV = VectorUtils.getAngleLimitedUnitVectorDegs(thisBoneInnerToOuterUV, mBaseboneRelativeConstraintUV, constraintAngleDegs);
                        }
                    ...
                    } else if (mBaseboneConstraintType == BaseboneConstraintType.GLOBAL_HINGE) {
                        IKJoint thisJoint = thisBone.getJoint();
                        ...
                        Vector3f thisBoneInnerToOuterUV = VectorUtils.projectOntoPlane(thisBone.getDirectionUV(), hingeRotationAxis);

                        // If we have a global hinge which is not freely rotating then we must constrain about the reference axis
                        if (!(Utils.approximatelyEquals(cwConstraintDegs, -IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.01f) && Utils.approximatelyEquals(acwConstraintDegs, IKJoint.MAX_CONSTRAINT_ANGLE_DEGS, 0.01f))) {
                    ...
                    } else if (mBaseboneConstraintType == BaseboneConstraintType.LOCAL_HINGE) {
                        IKJoint thisJoint = thisBone.getJoint();
                        Vector3f hingeRotationAxis = mBaseboneRelativeConstraintUV;                   // Basebone relative constraint is our hinge rotation axis!
                        float cwConstraintDegs = -thisJoint.getHingeClockwiseConstraintDegs();     // Clockwise rotation is negative!
                        float acwConstraintDegs = thisJoint.getHingeAnticlockwiseConstraintDegs();
		...
		*/
        ...
        return mChain.get(mChain.size() - 1).getEndLocation().distance(target);
    }
    ...
    private List<IKBone> cloneIkChain() {
        ...
        for (IKBone aBone : this.mChain) {
            ...
            clonedChain.add(new IKBone(aBone));
        }
    ...
    public int hashCode() {
        ...
        result = prime * result + ((mBaseboneRelativeReferenceConstraintUV == null) ? 0 : mBaseboneRelativeReferenceConstraintUV.hashCode());
